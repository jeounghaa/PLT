		...
		$.ajax({
			...
			success: function(result) {
				...
				function drawChart () {
					...
					for (var i = 0; i < result.length; i++) {
						if (i <= 9) {
							data.push([String('0'+i+'시'), Number(result[i])]); // 01시로 찍기위해
						} else{
							data.push([String(i+'시'), Number(result[i])]);
						}
					}
					
					var line_data = google.visualization.arrayToDataTable(data); // 위에 push한 데이터를 받아서 객체 생성

				  var options = {
				    title : day.substring(0, 2) + '월 ' + day.substring(3,5) + '일 매출', // 타이틀
